#!/usr/bin/env python
"""Script para limpiar TODA la base de datos manteniendo solo el usuario admin"""
import os
import django
...
admin_email = input("\n📧 Ingresa el email del usuario admin a mantener: ").strip()
try:
    admin_user = User.objects.get(email=admin_email)
    print(f"✅ Usuario encontrado: {admin_user.first_name} {admin_user.last_name} ({admin_user.role})")
...
print(f"   Solo se mantendrá: {admin_user.email}")
...
# 7. Eliminar todos los usuarios excepto el admin
users_to_delete = User.objects.exclude(id=admin_user.id)
deleted_counts['Usuarios'] = users_to_delete.delete()[0]
...
print("\n📊 ESTADO FINAL:")
print(f"  • Usuarios: {User.objects.count()} (solo {admin_user.email})")
print(f"  • Compañías: {Company.objects.count()}")
print(f"  • Programas: {Program.objects.count()}")
print(f"  • Participantes Programa: {ProgramParticipant.objects.count()}")
print(f"  • Vinculaciones: {Vinculation.objects.count()}")
...
print("\n" + "=" * 60)
print("✨ Base de datos limpia - Solo permanece el usuario admin")
print("=" * 60)
