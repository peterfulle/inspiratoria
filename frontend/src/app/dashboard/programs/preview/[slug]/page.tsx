...
import { ProgramTemplate, Module, SessionDetail, Resource, Activity, Milestone } from "../../types";

// ═══════════════════════════════════════════════════════════════════
// PREVIEW PAGE - Vista pública del programa
// ═══════════════════════════════════════════════════════════════════
...
export default function ProgramPreviewPage() {
  ...
  const template = useMemo(() => {
    return initialTemplates.find((t) => t.slug === slug);
  }, [slug]);
      ...
      <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#FAFAFA" }}>
        <div style={{ textAlign: "center", maxWidth: 400 }}>
          ...
          <button
            ...
            style={{ padding: "10px 24px", background: "#1a1a1a", color: "#fff", borderRadius: 8, border: "none", cursor: "pointer", fontSize: 14, fontWeight: 600 }}
          >
            ← Volver a Programas
          </button>
        </div>
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#FAFAFA" }}>
      {/* Top Bar */}
      <div style={{ background: "#fff", borderBottom: "1px solid #eee", padding: "12px 24px", display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, zIndex: 50 }}>
        <button
          onClick={() => router.push("/dashboard/programs")}
          style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 16px", background: "#f5f5f5", border: "1px solid #e0e0e0", borderRadius: 8, cursor: "pointer", fontSize: 14, fontWeight: 500, color: "#333" }}
        >
          ...
          {template.tags.length > 0 && (
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginTop: 20 }}>
              {template.tags.map((tag) => (
                <span key={tag} style={{ padding: "4px 12px", borderRadius: 20, fontSize: 12, background: "rgba(255,255,255,0.15)", color: "rgba(255,255,255,0.8)" }}>
                  #{tag}
                </span>
              ))}
            </div>
      ...
      {/* Tabs */}
      <div style={{ background: "#fff", borderBottom: "1px solid #eee", position: "sticky", top: 53, zIndex: 40 }}>
          ...
          <div>
            ...
            {template.milestones.length > 0 && (
              <section style={{ marginBottom: 40 }}>
                <h2 style={{ fontSize: 20, fontWeight: 700, color: "#1a1a1a", marginBottom: 20, display: "flex", alignItems: "center", gap: 8 }}>
                  🏆 Hitos del Programa
                </h2>
                    ...
                    <div key={ms.id} style={{ position: "relative", marginBottom: 24 }}>
                      ...
                      <div style={{ background: "#fff", border: "1px solid #eee", borderRadius: 12, padding: 20 }}>
                        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
                          ...
                          <span style={{ fontSize: 12, color: "#999", fontWeight: 500, background: "#f5f5f5", padding: "4px 10px", borderRadius: 20 }}>
                            Semana {ms.week}
                          </span>
                        </div>
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 13, color: "#999" }}>
                          <span>📦</span>
                          <span>Entregable: {ms.deliverable}</span>
                        </div>
                      </div>
                    </div>
                  ))}
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 16 }}>
                {template.modules.map((mod, idx) => (
                  ...
                  >
                    ...
                    <p style={{ fontSize: 13, color: "#666", lineHeight: 1.5, marginBottom: 12, display: "-webkit-box", WebkitLineClamp: 3, WebkitBoxOrient: "vertical", overflow: "hidden" }}>
                      {mod.description}
                    </p>
              ...
              <div style={{ background: "#fff", border: "1px solid #eee", borderRadius: 12, overflow: "hidden" }}>
                {template.modules.map((mod) =>
                  mod.resources.map((res, idx) => (
                    ...
                    >
                      <span style={{ fontSize: 18 }}>{resourceTypeIcons[res.type]?.icon || "📄"}</span>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: 14, fontWeight: 500, color: "#1a1a1a" }}>{res.name}</div>
                        {res.description && <div style={{ fontSize: 12, color: "#999", marginTop: 2 }}>{res.description}</div>}
                      </div>
                      <span style={{ fontSize: 11, color: "#999", padding: "2px 8px", background: "#f5f5f5", borderRadius: 4, textTransform: "uppercase", fontWeight: 600 }}>
                        {res.type}
                      </span>
                      {res.size && <span style={{ fontSize: 12, color: "#bbb" }}>{res.size}</span>}
                    </div>
                  ))
                )}
              </div>
        ...
        {activeTab === "modules" && (
          <div>
            <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginBottom: 20 }}>
              <button onClick={expandAll} style={{ padding: "6px 14px", fontSize: 13, border: "1px solid #e0e0e0", borderRadius: 8, background: "#fff", cursor: "pointer", color: "#666" }}>
                Expandir todo
              </button>
              <button onClick={collapseAll} style={{ padding: "6px 14px", fontSize: 13, border: "1px solid #e0e0e0", borderRadius: 8, background: "#fff", cursor: "pointer", color: "#666" }}>
                Colapsar todo
              </button>
            </div>
                    ...
                    <div style={{ background: "#fff", border: "1px solid #eee", borderTop: "none", borderRadius: "0 0 12px 12px", padding: "24px 20px" }}>
                      ...
                      {mod.objectives.length > 0 && (
                        <div style={{ marginBottom: 28 }}>
                          <h4 style={{ fontSize: 14, fontWeight: 600, color: "#1a1a1a", marginBottom: 12, display: "flex", alignItems: "center", gap: 6 }}>
                        ...
                        <div style={{ marginBottom: 28 }}>
                          ...
                          <div style={{ display: "grid", gap: 8 }}>
                            {mod.resources.map((res) => {
                              ...
                              return (
                                <div key={res.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 14px", border: "1px solid #f0f0f0", borderRadius: 8, background: "#FAFAFA" }}>
                    ...
                    {[
                      ...
                      <div key={w.label} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        ...
                        <div style={{ flex: 1, background: "#f5f5f5", borderRadius: 4, height: 8, overflow: "hidden" }}>
                          <div style={{ width: `${w.value}%`, height: "100%", background: w.color, borderRadius: 4, transition: "width 0.3s ease" }} />
                        </div>
