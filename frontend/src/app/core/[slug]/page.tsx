...
export default function CoreDashboardPage() {
  ...
  const handleAddParticipant = async () => {
    ...
    try {
      const uRes = await apiFetch(`${API_URL}/api/programs/users`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email: newParticipant.email, first_name: newParticipant.first_name, last_name: newParticipant.last_name }) });
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (companyStr) {
        ...
        // Validate slug matches the user's company
        if (slug && parsedCompany?.slug && slug !== parsedCompany.slug) {
          router.replace(`/core/${parsedCompany.slug}`);
          return;
        }
  ...
  const navItems = [
    ...
    { section: 'Gestion', items: [
      { id: 'manage-programs', label: 'Gestionar Programas', count: stats.programs },
              ...
              <div key={section.section} className="core-nav-section">
                ...
                {section.items.map(item => (
                  <button key={item.id} className={`core-nav-item ${activeNav === item.id ? 'active' : ''}`}
                    onClick={() => { setActiveNav(item.id); if (item.id !== 'programs' && item.id !== 'manage-programs') setSelectedProgram(null); }}>
                    {item.label}
