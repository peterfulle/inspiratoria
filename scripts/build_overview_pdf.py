"""Generate a black & white PDF from PROJECT_OVERVIEW.md."""
import re
...
OUT = ROOT / "PROJECT_OVERVIEW.pdf"

# Pure black & white styling (no color, no background fills, monochrome accents)
CSS = """
...
def main() -> None:
    ...
    # Strip internal anchor links ([text](#anchor)) — keep just the text — to
    # avoid pymupdf KeyError when destinations don't match heading slugs.
    md_text = re.sub(r"\[([^\]]+)\]\(#[^)]+\)", r"\1", md_text)

    pdf = MarkdownPdf(toc_level=2, optimize=True)
    pdf.meta["title"] = "Inspiratoria — Informe técnico A → Z"
    pdf.meta["author"] = "Inspiratoria"
    pdf.meta["subject"] = "Project overview & technical report"
